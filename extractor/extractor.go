...
func SelectVariantByQuality(variants []StreamVariant, quality int) string {
	if len(variants) == 0 {
		return ""
	}

	targetQuality := fmt.Sprintf("%dp", quality)

	for _, variant := range variants {
		resQuality := FormatResolutionQuality(variant.Resolution)
		if resQuality == targetQuality {
			return variant.URL
		}
	}
	...
	for _, variant := range variants {
		resQuality := FormatResolutionQuality(variant.Resolution)
		if resQuality == "720p" && quality == 360 {
			return variant.URL
		}
		if resQuality == "1080p" && quality == 720 {
			return variant.URL
		}
	}
...
func GetStreamURL(imdbID string, season, episode, quality int) (string, error) {
	mediaType := Movie
	...
	return SelectVariantByQuality(variants, quality), nil
}
